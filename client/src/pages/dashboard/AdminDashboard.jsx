...
const AdminDashboard = () => {
  ...
  const handleApproveForm = (formId) => {
  ...
  const currentTeacherForms = selectedTeacher?._id ? forms[selectedTeacher._id] || [] : [];
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-600 text-white font-black text-xl flex items-center justify-center shadow-md shadow-emerald-200">
            F
          </div>
          <div>
            <h1 className="font-bold text-slate-800 text-lg leading-tight">Admin Portal</h1>
            <p className="text-xs text-slate-500 font-medium">Manage Teachers, Form Approvals & Responses</p>
          </div>
        </div>

        {/* Action: Create Form */}
        <button className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl font-bold text-xs transition-all shadow-sm active:scale-95">
        ...
        <div className="md:col-span-4 space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
            ...
            <div className="space-y-2 pt-1">
              {teachers.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-4">No activated teachers found.</p>
              ) : (
                teachers
                  .filter((t) =>
                  ...
                  .map((teacher) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <p className={`font-bold text-xs ${isSelected ? "text-emerald-900" : "text-slate-800"}`}>
                            {teacher.fullName}
                          </p>
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                  <div>
                    ...
                    <h2 className="text-lg font-bold text-slate-800 mt-1">{selectedTeacher.fullName}</h2>
                  ...
                  <div className="flex items-center gap-3 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <div className="text-center px-2">
                      <p className="text-xs font-extrabold text-slate-800">{currentTeacherForms.length}</p>
                      <p className="text-[10px] text-slate-400">Total Forms</p>
                    </div>
                    <div className="w-px h-6 bg-slate-200"></div>
                    <div className="text-center px-2">
                      <p className="text-xs font-extrabold text-amber-600">
                        {currentTeacherForms.filter((f) => f.status === "pending").length}
                      </p>
                ...
                <div className="space-y-4">
                  ...
                  {currentTeacherForms.length === 0 ? (
                    <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl">
                      ...
                      <p className="text-xs font-semibold text-slate-500">Is teacher ne abhi tak koi form nahi banaya hai.</p>
                    </div>
                  ) : (
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <h4 className="font-bold text-slate-800 text-sm">{form.title}</h4>
                            <span
                          ...
                          <div className="flex items-center gap-4 text-xs text-slate-500">
                            <span>Batch: <strong className="text-slate-700">{form.batch}</strong></span>
                            <span>•</span>
                        ...
                        <div className="flex items-center gap-2 self-start sm:self-center">
                          ...
                          {form.status === "approved" && (
                            ...
                            >
                              ...
                              View Responses ({form.responsesCount})
                            </button>
                          )}
                        </div>
                      </div>
